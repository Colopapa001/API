  ...
  useEffect(() => {
    const restoreSession = async () => {
      try {
        ...
        if (sessionToken && !token.isExpired(sessionToken)) {
          ...
          if (userData && userData.email) {
            try {
              ...
              const result = await loginUser(userData.email, 'skip-password-check');
              if (result.success || userData.email) {
                // Verify user exists in database by fetching users
        ...
        // Si no hay token válido o usuario no existe, cerrar sesión
        token.remove();
