...
export const CartProvider = ({ children }) => {
  ...
  // Función para obtener todos los productos (API + memoria) - con refresh
  const getAllAvailableProducts = () => {
    // Get in-memory products from sessionStorage (refresh each time)
    const inMemoryProducts = JSON.parse(sessionStorage.getItem('inMemoryProducts') || '[]');
    return [...products, ...inMemoryProducts];
  };

  // Función para refrescar productos en memoria
  const refreshInMemoryProducts = () => {
    const inMemoryProducts = JSON.parse(sessionStorage.getItem('inMemoryProducts') || '[]');
  ...
  useEffect(() => {
    const loadProductsAndCart = async () => {
      try {
        ...
        // Cargar carrito desde localStorage usando la clave específica del usuario
        ...
        if (savedCart) {
          ...
          // Get all available products (API + in-memory)
          const inMemoryProducts = JSON.parse(sessionStorage.getItem('inMemoryProducts') || '[]');
          const allAvailableProducts = [...productsData, ...inMemoryProducts];
          
          // Validar que los productos aún existen y actualizar datos
          const validatedItems = cartItems.map(item => {
            const currentProduct = allAvailableProducts.find(p => p.id === item.productId);
            if (currentProduct) {
              return {
                ...item,
  ...
  // Limpiar carrito cuando el usuario cambie (logout/login)
  useEffect(() => {
    if (user) {
      ...
      if (savedCart) {
        ...
        // Get fresh products from both sources
        const loadUserCart = async () => {
          try {
            const apiProducts = await getAllProducts();
            const inMemoryProducts = refreshInMemoryProducts();
            const allAvailableProducts = [...apiProducts, ...inMemoryProducts];
  ...
  const addToCart = (product, quantity = 1) => {
    try {
      ...
      // Buscar producto actualizado en todos los productos disponibles (refresh)
      const allAvailableProducts = getAllAvailableProducts();
      const currentProduct = allAvailableProducts.find(p => p.id === product.id) || product;

      // Ensure the product has proper image data for display
      if (!currentProduct.image && !currentProduct.images) {
        currentProduct.image = 'https://via.placeholder.com/400x400/CCCCCC/666666?text=Sin+imagen';
      }
